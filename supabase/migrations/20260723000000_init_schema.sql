...
-- -> public write surface (rate limits) -> order audit triggers -> retention.
...
--
-- Kept in step with 20260727000013_retention_and_erasure.sql. The privacy
-- policy in src/data/legalContent.js promises an unsubscribe link (§6) and a
-- right to erasure (§6), while §4 requires order data to be kept for as long as
-- accounting law demands — so an erasure removes the person from the financial
-- record rather than deleting the record.
--
-- This section sits last because erase_order_pii() needs is_admin() (section 4)
-- and `orders` (section 2).
-- ============================================================================
